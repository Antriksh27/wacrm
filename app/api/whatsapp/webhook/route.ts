...
async function parseMessageContent(
  ...
  const verifyAndBuildUrl = async (
    ...
    try {
      ...
      // The message still has a valid Meta media ID. Do not turn a temporary
      // Graph API resolution failure into a NULL media_url, because the inbox
      // would render that as "Photo unavailable" and lose the ability to
      // retry the media fetch from the browser. The authenticated proxy can
      // resolve the media on demand.
      return `/api/whatsapp/media/${mediaId}`
    }
  }
